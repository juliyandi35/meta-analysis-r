...
library(metafor)
library(tidyr)
dir.create("Forest Plots MD", showWarnings = FALSE)
dir.create("Funnel Plots MD", showWarnings = FALSE)
dir.create("Leave-One-Out MD", showWarnings = FALSE)
dir.create("Baujat Plots MD", showWarnings = FALSE)
dir.create("Influence Diagnostic MD", showWarnings = FALSE)
dir.create("Significance Tables MD", showWarnings = FALSE)
...
run_analysis <- function(file_path) {
  ...
  write.xlsx(
    result_table,
    file = paste0("Significance Tables MD/", outcome, " summary_effect.xlsx"),
    overwrite = TRUE
  )
  
  pdf(paste0("Forest Plots MD/forest plot ", outcome, ".pdf"), width = 10, height = 7)
  par(mar = c(0,0,0,0))
  forest(meta_result,
         xlab = paste("Mean Difference (", outcome, ")", sep = ""),
         overall = TRUE,
         subgroup = TRUE,
         test.subgroup = TRUE,       # p-value antar subgroup
         overall.hetstat = TRUE,     # I² overall
         print.I2 = TRUE,            # I² per subgroup
         print.tau2 = TRUE,
         fontsize = 9)
  dev.off()
  
  pdf(paste0("Funnel Plots MD/funnel plot ", outcome, ".pdf"), 15, 20)
  par(mar = c(0,0,0,0))
  ...
  write.xlsx(
    loo_df,
    file = paste0("Leave-One-Out MD/", outcome, " leave1out.xlsx"),
    overwrite = TRUE
  )
  
  pdf(paste0("Leave-One-Out MD/leave1out plot ", outcome, ".pdf"), width = 10, height = 7)
  par(mar = c(0,0,0,0))
  forest(
    loo_result,
    xlab = paste("MD after omitting one study"),
    fontsize = 9
  )
  dev.off()
  ...
  # Baujat plot
  # -------------------------
  pdf(paste0("Baujat Plots MD/baujat plot ", outcome, ".pdf"), 10, 8)
  baujat(meta_result)
  dev.off()
  ...
  png(
    filename = paste0("Influence Diagnostic MD/influence_plot_", outcome, ".png")
  )
  plot(inf)
  dev.off()
